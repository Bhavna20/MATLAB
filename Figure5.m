...
TrivarP; %Calls the regression function defined in TrivarP


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% VAR Impulse response analysis 
[IRF]=irfvar(A,SIGMA(1:q,1:q),p,h);
    IRF(1,:)=cumsum(IRF(1,:)); % Since oil production was in percentage change, we take the cumulative sum for oil production shocks
    IRF(4,:)=cumsum(IRF(4,:));
    IRF(7,:)=cumsum(IRF(7,:));
...
for r=1:nrep
    ...
    eta=[eta; eta; eta];
	Ur(1:q,2:t-p+1)=U(1:q,:).*eta;	%Bootstrap reduced form errors
